...
$q1="SELECT kode_perusahaan FROM tb_perusahaan WHERE kode_perusahaan IN (SELECT kode_perusahaan FROM tb_permohonan JOIN tb_detail_permohonan ON tb_permohonan.no_surat_permohonan=tb_detail_permohonan.no_surat_permohonan WHERE status_permohonan='1')";
...
?>
<div class="container">
	<h5>Data Keseluruhan Perusahaan</h5><hr>
	<!--<div class="dropdown">
        <button class="btn btn-outline-info btn-sm" data-toggle="dropdown">Data Angket Jurusan</button>
           <div class="dropdown-menu">
              <?php 
              //$query="SELECT * FROM tb_jurusan";
              //$result=mysql_query($query) or die(mysql_error());
              while ($hasil=mysql_fetch_array($result)) { ?>
                <a class="dropdown-item" href="data_angket-jurusan.php?j=<?php echo $hasil['kode_jurusan'];?>"><?php echo $hasil['nama_jurusan'];?></a>
              <?php
              }
              ?>
...
<form method="POST" name="angket">
	...
	<?php
		...
		echo "<b>Jumlah Skor Ideal</b><br>";
		echo "Skor Tertinggi : ". $batasAtas ."<br>";
		echo "Skor Terendah : ". $batasBawah ."<br>";
	?>
	<p>
	</form>
	<p>
...
<?php
//((SS*5)+(S*4)+(N*3)+(TS*2)+(STS*1))/(5*3)
		$skill=((($SS1*5)+($S1*4)+($H1*3)+($TS1*2)+($STS1*1))/$batasAtas)*100;		
		$tanggungJawab=((($SS2*5)+($S2*4)+($H2*3)+($TS2*2)+($STS2*1))/$batasAtas)*100;	
		$motivasi=((($SS3*5)+($S3*4)+($H3*3)+($TS3*2)+($STS3*1))/$batasAtas)*100;
		$kerjasama=((($SS4*5)+($S4*4)+($H4*3)+($TS4*2)+($STS4*1))/$batasAtas)*100;
		$perilaku=((($SS5*5)+($S5*4)+($H5*3)+($TS5*2)+($STS5*1))/$batasAtas)*100;
		$disiplin=((($SS6*5)+($S6*4)+($H6*3)+($TS6*2)+($STS6*1))/$batasAtas)*100;
		switch ($skill) {
			case ($skill<=20):
